...
- 底层手写架构，仅依赖基本的glibc函数之外的最小外部库，以实现最大灵活性和最小开销，可以在任何Linux安装上构建和部署。
...
在任何Linux安装上，构建abcmint-ckpool不需要基本构建工具和yasm之外的依赖项。推荐的zmq通知支持（仅限ckpool）需要安装zmq开发库。
...
ckpool - 主矿池后端，已适配abcmint的rainbow18算法
...
-B将以独立挖矿模式启动ckpool，专为独立挖矿设计。所有连接的用户名必须是有效的abcmint地址，100%的区块奖励将归属于解决区块的用户，减去任何设置的捐款。
...
"btcaddress"：这是尝试生成区块的abcmint地址（以'8'开头）。在独立挖矿模式下被忽略。
...
1. 区块版本：abcmint使用区块版本号101（ABCMINT_BLOCK_VERSION），ckpool已配置为强制使用此版本。

2. 挖矿算法：abcmint使用rainbow18后量子加密算法，而非比特币的SHA-256算法。
